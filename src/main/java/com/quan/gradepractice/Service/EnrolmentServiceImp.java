...
public class EnrolmentServiceImp implements EnrolmentService {
    ...
    private Enrolment checkEnrolment(Long studentId, Long courseId, Optional<Enrolment> entity) {
        ...
        throw new EntityNotFoundException("the enrolment with the student id " + studentId + " and the course id " + courseId + " not found");
    }
    ...
    public Enrolment saveEnrolment(Long studentId, Long courseId) {
       ...
       if(entity.isPresent()) {
        ...
        throw new EntityNotFoundException("the enrolment with the student id " + studentId + " and the course id " + courseId + "  already exists");
       }

        
        Optional<Student> studentEntity = studentRepo.findById(studentId);
        if(!studentEntity.isPresent()) throw new EntityNotFoundException("the student id " + studentId + " not found");
        Student student = studentEntity.get();
        Optional<Course> courseEntity = courseRepo.findById(courseId);
        if(!courseEntity.isPresent()) throw new EntityNotFoundException("the course id "+ courseId + " not found");
